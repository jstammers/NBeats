...
class NBeatsBlock(tf.keras.layers.Layer):
    ...
    def create_output_weights(
        ...
        if self.stack_weights == StackWeight.trend:
            ...
            forecast_weights = [
                np.ones_like(forward_tvec[:, np.newaxis]),
            ]
    ...
    def seasonal_coeffs(time_vec, max_time):
        ...
        cos = np.cos(2 * np.pi * np.arange(1, np.floor(max_time/2))[:, np.newaxis] * time_vec )
        sin = np.sin(2 * np.pi  * np.arange(1, np.floor(max_time/2))[:, np.newaxis] * time_vec)
